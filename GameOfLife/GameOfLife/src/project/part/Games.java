...
public class Games {
	...
	private final String BUNDLENAME= new String("project.part.string");
	...
	public void lapAfterLap() throws java.io.IOException
	{
		...
		// he ca, modify a cell during simulation
		...
		while ( next == 1 )  
		{
			...
			if ( difficulty == 1 ) // then the user can change the color of a current cell
			{
					...
					while(next == 1)
					{
						...
						if ( life.getCurrentCell(line, column).equals(Color.black) )
						{
							
							life.setCurrentCell(Color.white, line-1, column-1);
							life.setUpdatingCell(Color.white, line-1, column-1);
			...
			while ( next != 0 && next != 1 )  // We ask the user if he wants to go to the next lap
			{
