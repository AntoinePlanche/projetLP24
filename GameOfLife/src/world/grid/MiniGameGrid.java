...
import world.cell.MiniGameCell;
...
public class MiniGameGrid {
	...
	public MiniGameGrid()
	{
		...
		this.line = 1000;
		this.column = 1000;
		grid = new MiniGameCell[line][column];
		...
		for ( int i = 0; i < this.line; i++ )
		{
			...
			for ( int j = 0; j < this.column; j++ )
			{
				...
				grid[i][j] = new MiniGameCell(); // dynamically initiate the grid
	...
	public MiniGameGrid(int a, int b)
	{
		...
		if ( language.equals("en") )
		{
			...
			for ( int i = 0; i < this.line; i++ )
			{
				...
				for ( int j = 0; j < this.column; j++ )
				{
					...
					System.out.println(
							"the cell[" + (i + 1) + "][" + (j + 1) + "] is " + this.grid[i][j].printCell(language));
		...
		else
		{
			...
			for ( int i = 0; i < this.line; i++ )
			{
				...
				for ( int j = 0; j < this.column; j++ )
				{
					...
					System.out.println(
							"la cellule[" + (i + 1) + "][" + (j + 1) + "] est " + this.grid[i][j].printCell(language));
	...
	public MiniGameGrid lapAfterLapGraphicMiniGame(FileWriter writer, int lap, int goalLap,
			ResourceBundle RESOURCEBUNDLE) throws IOException
	{
		...
		for ( int i = 1; i < (this.getLine() - 1); i++ )
		{
			...
			for ( int j = 1; (j < this.getColumn() - 1); j++ )
			{
				...
				/*
				 * I check if the cell is black or white then I calculate the number of black neighbors with the function getCurrentValue (see class cell line 92 for
				 * more details) and I apply the appropriate treatment (the cell becomes black or white). Note that we could first calculate the number of black
				 * neighbors and then test if the cell is white or black. It comes down to the same level of complexity
				 */

				if ( this.getCurrentCell(i, j).equals(Color.black) )
				{

					NbrNeighbour = this.getValue((i - 1), (j - 1)) + this.getValue((i - 1), (j))
							+ this.getValue((i - 1), (j + 1)) + this.getValue((i), (j - 1))
							+ this.getValue((i), (j + 1)) + this.getValue((i + 1), (j - 1))
		...
		for ( int i = 1; i < this.getLine() - 1; i++ )
		{
...

			{
				...
				NbrNeighbour = this.getValue((i - 1), (this.getColumn() - 1))
						+ this.getValue((i - 1), (this.getColumn() - 2)) + this.getValue((i), (this.getColumn() - 2))
						+ this.getValue((i + 1), (this.getColumn() - 2))
		...
		for ( int j = 1; j < this.getColumn() - 1; j++ )
		{
			...
			if ( this.getCurrentCell(this.getLine() - 1, (j)).equals(Color.black) )
			{
				...
				NbrNeighbour = this.getValue((this.getLine() - 1), (j - 1))
						+ this.getValue((this.getLine() - 2), (j - 1)) + this.getValue((this.getLine() - 2), (j))
						+ this.getValue((this.getLine() - 2), (j + 1)) + this.getValue((this.getLine() - 1), (j + 1));
		...
		if ( this.getCurrentCell(this.getLine() - 1, this.getColumn() - 1).equals(Color.black) )
		{
			...
			NbrNeighbour = this.getValue(this.getLine() - 2, this.getColumn() - 1)
					+ this.getValue(this.getLine() - 2, this.getColumn() - 2)
		...
		if ( lap > goalLap ) // then we lose
		{
			...
			JOptionPane.showMessageDialog(null, RESOURCEBUNDLE.getString("keytwentyseven"),
		...
		else
		{
			...
			for ( int i = 0; i < this.getLine(); i++ )
			{
				...
				for ( int j = 0; j < this.getColumn(); j++ )
				{
					...
					if ( this.getCurrentCell(i, j).equals(this.getGoalCell(i, j)) )
					{
			...
			if ( counter == (this.getLine() * this.getColumn()) ) // if the condition is true then it's the victory !!
			{ // no verification in the first lap, so we canot win in the first lap

				JOptionPane.showMessageDialog(null, RESOURCEBUNDLE.getString("keytwentyfour") + lap,
						RESOURCEBUNDLE.getString("keygraphiceleven"), JOptionPane.INFORMATION_MESSAGE);
				writer.write(RESOURCEBUNDLE.getString("keytwentysix") + " " + this.numberOfCurrentBlackCell() * 50
