...
public class MiniGameGrid {
	...
	public  MiniGameGrid lapAfterLapGraphicMiniGame(FileWriter writer, int lap, int goalLap, ResourceBundle RESOURCEBUNDLE) throws IOException 
	{
		...
		if ( lap > goalLap ) // then we lose
		{
			...
			JOptionPane.showMessageDialog(null, RESOURCEBUNDLE.getString("keytwentyseven"), RESOURCEBUNDLE.getString("keygraphicfive"), JOptionPane.INFORMATION_MESSAGE);
		...
		else
		{
			...
			{													// no verification in the first lap, so we canot win in the first lap
				...
				JOptionPane.showMessageDialog(null, RESOURCEBUNDLE.getString("keytwentyfour") + lap, RESOURCEBUNDLE.getString("keygraphiceleven"), JOptionPane.INFORMATION_MESSAGE);
				writer.write(RESOURCEBUNDLE.getString("keytwentysix") + " " + this.numberOfCurrentBlackCell()*50 + " points.\n"); // We mark the score in the file
				writer.write(RESOURCEBUNDLE.getString("keytwentyseven"));
				writer.close();
				System.exit(0);
