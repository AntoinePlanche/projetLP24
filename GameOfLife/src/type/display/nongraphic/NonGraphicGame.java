...
// This class is useful in the case of the non-graphic application
...
public class NonGraphicGame {
	...
	private byte miniGames; // same reason
	public byte typeOfGraphique; // same reason, public for the main class access without getter
	
	// Constructor
	
	public NonGraphicGame(ResourceBundle RESOURCEBUNDLE) 
	{
		...
		// Now it's the part where the player/user choose it's start configuration
	...
	public void lapAfterLapNonGraphic() throws java.io.IOException, Exception
	{
		...
		int lap = 1;
		int NbrNeighbour = 0;
		char next = 0; // char to avoid taking up too much memory
		...
		else //so miniGames = 2, the code below it's the same than above but it's just a simulation, the player can't interfere
		{
