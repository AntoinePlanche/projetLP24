		...
		while ( next == 1 )
		{
			...
			for ( int i = 1; i < (life.getLine() - 1); i++ )
			{
				...
				for ( int j = 1; (j < life.getColumn() - 1); j++ )
				{
					...
					/*
					 * I check if the cell is black or white then I calculate the number of black neighbors with the function getCurrentValue (see class Cell line 96 for
			...
			for ( int j = 1; j < life.getColumn() - 1; j++ )
			{
				...
				if ( life.getCurrentCell(life.getLine() - 1, (j)).equals(Color.black) )
				{
					...
					NbrNeighbour = life.getValue((life.getLine() - 1), (j - 1))
							+ life.getValue((life.getLine() - 2), (j - 1)) + life.getValue((life.getLine() - 2), (j))
			...
			else
			{
				...
				{ // no verification in the first lap, so we canot win in the first lap
					...
					System.out.println(RESOURCEBUNDLE.getString("keytwentyfour") + " " + lap
							+ RESOURCEBUNDLE.getString("keytwentyfive"));
					writer = new FileWriter("resources\\information\\score.txt", true);
					writer.write(RESOURCEBUNDLE.getString("keytwentysix") + " " + life.numberOfCurrentBlackCell() * 50
