...
public class NonGraphicSimulation {
	...
	/*
	 * This below class allow us to activate a second execution thread to interrupt the simulation by a keyboard input
	 */
	...
	public class ThreadSimulation extends Thread // nested class
	{
		...
		public void run()
		{
			...
			System.out.println(RESOURCEBUNDLE.getString("keythirtyseven") + life.numberOfCurrentBlackCell() + " "
					+ RESOURCEBUNDLE.getString("keythirtyeight"));
			...
			while ( !isInterrupted() ) // As long as the thread is not interrupted ...
			{
				...
				for ( int i = 1; i < (life.getLine() - 1); i++ )
				{
					...
					for ( int j = 1; (j < life.getColumn() - 1); j++ )
					{
						...
						if ( life.getCurrentCell(i, j).equals(Color.black) )
						{
							...
							NbrNeighbour = life.getValue((i - 1), (j - 1)) + life.getValue((i - 1), (j))
									+ life.getValue((i - 1), (j + 1)) + life.getValue((i), (j - 1))
									+ life.getValue((i), (j + 1)) + life.getValue((i + 1), (j - 1))
				...
				for ( int i = 1; i < life.getLine() - 1; i++ )
				{
					...
					if ( life.getCurrentCell(i, 0).equals(Color.black) )
					{
						...
						NbrNeighbour = life.getValue((i - 1), (0)) + life.getValue((i - 1), (1))
								+ life.getValue((i), (1)) + life.getValue((i + 1), (0)) + life.getValue((i + 1), (1));
					...
					if ( life.getCurrentCell(i, (life.getColumn() - 1)).equals(Color.black) )
					{
						...
						NbrNeighbour = life.getValue((i - 1), (life.getColumn() - 1))
								+ life.getValue((i - 1), (life.getColumn() - 2))
								+ life.getValue((i), (life.getColumn() - 2))
								+ life.getValue((i + 1), (life.getColumn() - 2))
				...
				for ( int j = 1; j < life.getColumn() - 1; j++ )
				{
					...
					if ( life.getCurrentCell(0, j).equals(Color.black) )
					{
						...
						NbrNeighbour = life.getValue((0), (j - 1)) + life.getValue((1), (j - 1))
								+ life.getValue((1), (j)) + life.getValue((1), (j + 1)) + life.getValue((0), (j + 1));
					...
					if ( life.getCurrentCell(life.getLine() - 1, (j)).equals(Color.black) )
					{
						...
						NbrNeighbour = life.getValue((life.getLine() - 1), (j - 1))
								+ life.getValue((life.getLine() - 2), (j - 1))
								+ life.getValue((life.getLine() - 2), (j))
								+ life.getValue((life.getLine() - 2), (j + 1))
				...
				if ( life.getCurrentCell(life.getLine() - 1, life.getColumn() - 1).equals(Color.black) )
				{
					...
					NbrNeighbour = life.getValue(life.getLine() - 2, life.getColumn() - 1)
							+ life.getValue(life.getLine() - 2, life.getColumn() - 2)
